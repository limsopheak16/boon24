...
`.trim();
  ...
  const quality = modelQuality === 'Pro' ? 'hd' : 'standard';
  const size = '1024x1024'; // DALL-E 3 fixed size
  ...
  try {
    ...
    if (isTextToConcept) {
      ...
      imageUrl = await generateImage(promptText, quality, size);
    } else {
      // For now, we'll use text-to-image even with input images
      // DALL-E 3 doesn't support image-to-image directly
      // We could enhance this later with DALL-E 2 editing
      ...
      `;
      imageUrl = await generateImage(enhancedPrompt, quality, size);
    }

    return { images: [imageUrl], urls: [] };
  } catch (error: any) {
...
export const editImageWithAi = async (
    ...
    try {
      // For now, use text-to-image generation for editing as well
      // DALL-E 2 editing could be implemented later if needed
      const enhancedPrompt = `
        ARCHITECTURAL EDITING TASK:
        Based on the provided input image, perform the following modification:
        
        ${textPrompt}
        
        CRITICAL: Maintain the overall architectural style, lighting, and composition while making the requested changes.
      `;
      
      const imageQuality = quality === 'Pro' ? 'hd' : 'standard';
      const imageUrl = await generateImage(enhancedPrompt, imageQuality, '1024x1024');
